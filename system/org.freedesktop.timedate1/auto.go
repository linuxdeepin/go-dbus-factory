...
type timedate interface {
	...
	GoListTimezones(flags dbus.Flags, ch chan *dbus.Call) *dbus.Call
	ListTimezones(flags dbus.Flags) ([]string, error)
...
func (v *interfaceTimedate) GoListTimezones(flags dbus.Flags, ch chan *dbus.Call) *dbus.Call {
	return v.GetObject_().Go_(v.GetInterfaceName_()+".ListTimezones", flags, ch)
}

func (*interfaceTimedate) StoreListTimezones(call *dbus.Call) (timezones []string, err error) {
	err = call.Store(&timezones)
	return
}

func (v *interfaceTimedate) ListTimezones(flags dbus.Flags) ([]string, error) {
	return v.StoreListTimezones(
		<-v.GoListTimezones(flags, make(chan *dbus.Call, 1)).Done)
}
