...
func (v *sessionManager) GoSetLowPowerUI(flags dbus.Flags, ch chan *dbus.Call, enabled bool) *dbus.Call {
	return v.GetObject_().Go_(v.GetInterfaceName_()+".SetLowPowerUI", flags, ch, enabled)
}

func (v *sessionManager) SetLowPowerUI(flags dbus.Flags, enabled bool) error {
	return (<-v.GoSetLowPowerUI(flags, make(chan *dbus.Call, 1), enabled).Done).Err
}

// method SetBatteryWarnLevel

func (v *sessionManager) GoSetBatteryWarnLevel(flags dbus.Flags, ch chan *dbus.Call, level int32) *dbus.Call {
	return v.GetObject_().Go_(v.GetInterfaceName_()+".SetBatteryWarnLevel", flags, ch, level)
}

func (v *sessionManager) SetBatteryWarnLevel(flags dbus.Flags, level int32) error {
	return (<-v.GoSetBatteryWarnLevel(flags, make(chan *dbus.Call, 1), level).Done).Err
}
...
// property ShowLowPowerUI b

func (v *sessionManager) ShowLowPowerUI() proxy.PropBool {
